<template>
  <div class="box">
    <div class="box-bg box-bg-left"></div>
    <div class="box-bg box-bg-right"></div>
    ...
    <transition name="msg-type">
      <div class="msg-type" v-show="msgType">
        ...
        <div class="info">
          等级：
          <span>{{relation.level[nowShowData.level]}}</span>
        </div>
        <div class="info">
          原因：
    ...
    <transition name="msg-plain">
      <div class="msg-plain" v-show="msgPlain">
        <Icon type="load-a" :size="40" class="rotateAnimate" :style="{lineHeight: '85px', verticalAlign: 'middle', marginRight: '10px'}"></Icon>
...
export default {
  ...
  methods: {
    beginAnimateView(data) {
      if (data.length) {
        let first = data[0];
        if (first) {
          if (first.message_type == 2 || first.message_type == 3) {
            this.repairAnimate();
          } else {
            this.failureAnimate();
          }
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                this.msgContent = !this.msgContent;
                setTimeout(() => {
                  this.$store.commit("setMsgFailure", this.nowShowData);
                  this.data.shift();
                }, 2600);
              }, 3500);
            }, 1000);
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            this.repairLoading = Math.floor(Math.random() * 50);
            setTimeout(() => {
              this.repairLoading = 100;
              setTimeout(() => {
                this.msgLoadingtitle = "修复成功";
                this.$store.commit("setMsgRequire", this.nowShowData);
                setTimeout(() => {
                  this.msgCrossing = !this.msgCrossing;
                  this.msgType = !this.msgType;
                  this.msgLoading = !this.msgLoading;
    ...
    loadingView() {
      ...
      let count = this.repairLoading_origin;
      let finalNum = this.repairLoading;
      ...
      let timer = setInterval(() => {
        ...
        this.repairLoading_origin = count;
        render();
      }, 50);
    },
    // 实时请求新数据 5s
      ...
      setInterval(() => {
        this.$http("Ma_zong/controlCenter").then(res => {
          ...
          if (data.length) {
            ...
            this.allData.push(false);
            // 将重复的合并
            this.allData = Array.from(new Set(this.allData));
          }
        });
      }, 5000);
    }
...
.box {
  ...
  &-bg-right {
    ...
    &:before {
      margin-left: 76px;
      transform-origin: 291.5px 320.5px;
      animation: anim-circle 60s linear infinite reverse;
    }
  }
}
</style>
